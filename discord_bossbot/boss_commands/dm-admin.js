...
module.exports = {
    ...
    execute(bot, message, args, VARSET) {
      ...
      if(VARSET.adminRole == undefined || VARSET.adminRole == null) return message.channel.send("- Error: You have to set Admin role first using +setup command!",{code : "diff"});
      let msg = args[0];

      let admins = message.guild.roles.get(VARSET.adminRole).members.map(m=>m.user);
      let count = 0;
      admins.forEach(function(u, users){
        ...
        if(uStatus.presence.status != "offline" && u.id != VARSET.BOTID){
